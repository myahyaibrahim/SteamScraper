...
def SteamCrawlerMain(url, urlGenre):
    # Main Function
    # Scrapping game data from games listing on steam
    dataResult = requestData(url)
    gameList = parseDataSearchPage(dataResult)
    GameDictionary = {
        "GamesData" : gameList,
    }
    with open(os.path.dirname(__file__) + '/../data/SteamGame.json', 'w') as f:
        json.dump(GameDictionary, f, indent=2)
    
    # Scrapping genres of games availabe on steam
    genreList = getGenres(urlGenre)
    GenreDictionary = {
        "GenreData" : genreList,
    }
    with open(os.path.dirname(__file__) + '/../data/SteamGenre.json', 'w') as f:
        json.dump(GenreDictionary, f, indent=2)

SteamCrawlerMain(url, urlGenre)